...
<!--
  ...
  docs/metrics.md #1 and is no longer computed or acted on. (This example
  previously kept density_band_status: in-band live alongside
...
Every row here is `origin: legacy` with an empty `disposition` — this
behavior contains no known defect. See `templates/BHV-template.md` for the
`legacy-defect` case, where the scenario is true of the legacy system and
deliberately not reproduced.

Where each of these becomes observable in the target is not recorded in this
document — it is derived by `c7b` and lives in the pack's
`behaviors/scenario-bindings.json`. The rendered outputs in this directory
carry it as a `# surface:` comment for convenience only.
