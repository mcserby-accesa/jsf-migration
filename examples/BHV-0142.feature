Feature: BHV-0142 — Leave request detail submission
  ...
  #
  # The description is emitted as comments, not as Gherkin's free-text
  # narrative: a description line beginning with a step keyword is read by
  # real parsers as a stray step declaration and fails the whole file.

  @BHV-0142-S01
  Scenario: BHV-0142-S01 — the request is saved with status PENDING_MANAGER_APPROVAL and the user is navigated to the confirmation screen
  ...
  @BHV-0142-S03
  Scenario: BHV-0142-S03 — a validation error about the half-day flag is shown and the request is not saved
  ...
  @DT-BHV-0142-01
  Scenario Outline: DT-BHV-0142-01 — start/end date ordering check
